...
export class CardImageService {
  ...
  private determineStrategy(instructions: string): "fetch" | "generate" {
    ...
    const isConceptual = conceptualKeywords.some((kw) =>
      instructions.toLowerCase().includes(kw)
    );
    return isConceptual ? "generate" : "fetch";
  }

  private extractSearchQuery(instructions: string): string {
    // The prompt now encourages simple noun-based search terms, so we can trust the input more.
    // Just do basic cleanup.
    let query = instructions.trim();
    ...
    while (query !== previous) {
      ...
      query = query
        .replace(
          /^(show|display|find|get|generate|create|make|an image of|a picture of|a photo of|a photograph of|an illustration of|a diagram of|a|an|the)\s+/i,
          "",
        )
        .trim();
    }

    // If query is still very long (>100 chars), it might be a description rather than a search term.
    // Try to take the first sentence or clause.
    if (query.length > 100) {
      const firstSentence = query.split(/[.?!;]/)[0]; // Removed comma from split to avoid breaking lists
      if (firstSentence.length < 100) {
        query = firstSentence.trim();
      } else {
        // Fallback: truncate
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.log(
          ...
          "cards",
          "warn",
          "[image] failed to download card image",
          {
      ...
      const uploadResult = await this.supabase.storage.from(this.bucket).upload(
        path,
        buffer,
        {
          cacheControl: "3600",
      ...
      if (uploadResult.error) {
        this.logger.log(
          ...
          "cards",
          "warn",
          "[image] failed to upload card image",
          {
      ...
      const { data } = this.supabase.storage.from(this.bucket).getPublicUrl(
        path,
      );
      if (!data || !data.publicUrl) {
        this.logger.log(
          ...
          "cards",
          "warn",
          "[image] failed to resolve public url for card image",
          {
            path,
          },
        );
      ...
      this.logger.log(
        ...
        "cards",
        "warn",
        "[image] unexpected error caching remote image",
        {
          error: String(error),
          imageUrl,
        },
  ...
  async handleVisualRequest(
    ...
    const strategy: "fetch" | "generate" = request.strategy ||
      this.determineStrategy(request.instructions);

    // If explicit source_url provided, cache it directly
    if (
      request.strategy === "fetch" && typeof request.source_url === "string" &&
      request.source_url.trim().length > 0
    ) {
      const cachedUrl = await this.cacheRemoteImage(
        request.source_url.trim(),
        eventId,
        cardId,
      );
      if (cachedUrl) {
        const { calculateImageFetchCost } = await import(
          "../../../../lib/pricing"
        );
    ...
    if (strategy === "fetch") {
      if (!this.imageFetcher) {
        this.logger.log(
          ...
          "cards",
          "warn",
          "[image] Fetch requested but ImageFetcher not available",
        );
        return null;
      }
      ...
      this.logger.log(
        ...
        "cards",
        "log",
        "[image] Extracted search query",
        {
          original: request.instructions.substring(0, 150),
      ...
      if (!fetchResult.url) {
        this.logger.log(
          ...
          "cards",
          "warn",
          "[image] All fetch providers failed",
          {
            query: searchQuery,
          },
        );
      ...
      if (cachedUrl) {
        this.logger.log(
          ...
          "cards",
          "log",
          "[image] Fetched and cached image",
          {
    ...
    if (strategy === "generate") {
      if (!this.imageGenerator) {
        this.logger.log(
          ...
          "cards",
          "warn",
          "[image] Generate requested but ImageGenerator not available",
        );
        return null;
      }
      ...
      if (cachedUrl && genResult.cost > 0) {
        ...
        this.logger.log(
          ...
          "cards",
          "log",
          "[image] Generated and cached image",
          {
            model: genResult.model,
            cost: genResult.cost,
            latency: Date.now() - startTime,
