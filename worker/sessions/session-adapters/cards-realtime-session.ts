...
import { getCardsRealtimeTooling } from '../agent-profiles/cards/tooling';

/**
 * CardsRealtimeSession
 * Manages websocket lifecycle for the cards agent (OpenAI Realtime API).
 */
export class CardsRealtimeSession implements AgentRealtimeSession {
  ...
  constructor(openai: OpenAI, config: RealtimeSessionConfig) {
    if (config.agentType !== 'cards') {
      throw new Error(
        `CardsRealtimeSession expects agentType 'cards', received '${config.agentType}'`
      );
    }
  ...
  async connect(): Promise<string> {
    ...
    if (!this.config.model) {
      throw new Error('Missing realtime model for cards agent');
    }
