...
const DEFAULT_OUTPUT_PATH = '/tmp/generated-transcript.pcm';
...
async function main(): Promise<void> {
  ...
  if (!eventId && !skipStream) {
    console.error('[generate-pcm] error:', 'eventId is required when streaming to worker');
    process.exit(1);
  }

  if (!workerUrl && !skipStream) {
    console.error('[generate-pcm] error:', 'workerUrl is required when streaming to worker');
    process.exit(1);
  }
  ...
  for (const segment of transcript) {
    ...
    console.log(
      `[generate-pcm] Synthesizing speaker="${String(segment.speaker)}" voice="${voice}"`
    );
    ...
    const pauseDuration =
      typeof segment.pauseMs === 'number' && segment.pauseMs >= 0
        ...
        : defaultPauseMs;
    if (pauseDuration > 0) {
      console.log(`[generate-pcm] Inserting ${pauseDuration} ms pause`);
      audioChunks.push(
        Buffer.alloc(
  ...
  if (cliOptions.dryRun === true) {
    console.log('[generate-pcm] Dry run complete, no audio generated');
    return;
  }

  const combined = Buffer.concat(audioChunks);
  if (combined.length === 0) {
    console.error('[generate-pcm] error:', 'Synthesized audio payload is empty');
    process.exit(1);
  }
  ...
  console.log(`[generate-pcm] ✅ Wrote ${combined.length} bytes to ${outputPath}`);

  if (skipStream) {
    console.log('[generate-pcm] Skipping live stream (--skip-stream enabled)');
    return;
  }
...
function validateTranscript(transcript: TranscriptSegment[]): void {
  if (transcript.length === 0) {
    console.error('[generate-pcm] error:', 'Transcript template is empty');
    process.exit(1);
  }
...
async function streamToWorker(options: StreamOptions): Promise<void> {
  ...
  console.log(
    `[generate-pcm] 🚀 Streaming ${totalChunks} chunk(s) to worker transcript endpoint`
  );
  ...
  for (let offset = 0; offset < trimmedBuffer.length; offset += chunkByteLength) {
    ...
    console.log(
      `[generate-pcm] ✅ Sent chunk ${seq + 1}/${totalChunks} (${chunkBuffer.length} bytes, ~${chunkDurationMs} ms, seq=${seq})`
    );
  ...
  console.log('[generate-pcm] 📬 Completed live stream to worker transcript agent');
}
...
function parsePositiveInteger(
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    console.error('[generate-pcm] error:', `Invalid ${label}: ${rawValue}`);
    process.exit(1);
  }
...
void main().catch((err: unknown) => {
  console.error('[generate-pcm] error:', String(err));
  process.exit(1);
});
